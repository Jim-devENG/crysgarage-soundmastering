...
# Webhook Setup Script for VPS
# This script will automatically set up the GitHub webhook

echo "Setting up GitHub webhook..."

# Create webhook.php file
cat > /var/www/html/webhook.php << 'EOF'
<?php
...
chmod 644 /var/www/html/webhook.php
chown apache:apache /var/www/html/webhook.php

echo "Set permissions for webhook.php"

# Create log file
touch /var/log/webhook.log
chmod 666 /var/log/webhook.log

echo "Created webhook log file"

# Test the webhook
echo "Testing webhook..."
curl -X POST -H "Content-Type: application/json" \
  -d '{"ref":"refs/heads/main","repository":{"name":"crys-fresh"}}' \
  http://localhost/webhook.php

echo ""
echo "Webhook setup completed!"
echo "Check the response above to verify it's working."
echo "You can monitor logs with: tail -f /var/log/webhook.log" 